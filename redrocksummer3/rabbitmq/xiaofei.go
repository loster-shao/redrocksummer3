...
func OpenConsumer() {
	...
	tool.HandleError(err, "Can't create a amqpChannel")//检测错误
	...
	tool.HandleError(err, "Could not declare `add` queue")//检测错误

	err = amqpChannel.Qos(1, 0, false)//TODO 这是啥？
	tool.HandleError(err, "Could not configure QoS")//检测错误
	...
	tool.HandleError(err, "Could not register consumer")//检测错误
	...
	go func() {
		...
		for d := range messageChannel {
			...
			log.Println("Good:", string(d.Body))//打印日志

			model.Order(string(d.Body))
			//model.Buy(string(d.Body))
